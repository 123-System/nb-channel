...
### 2. 虚拟股票交易系统（新功能！）✅
- **买入/卖出**：表格新增股价列和交易按钮，点击弹出交易面板
  （显示股价/持仓/余额/预计金额含手续费，买入红色、卖出绿色）
- **手续费**：买卖双向 **5%**（买入=交易额×1.05 支付，卖出=交易额×0.95 到账），
  手续费记录在 `transactions.fee`，不进入公司市值
- **我的持仓**：持仓卡片展示持仓公司、股数、成本价、现价、浮动盈亏、持仓市值
- **后端增强**（`sql/stock_trading.sql`）：
  - `buy_stock`/`sell_stock` 增强：**禁止买入自己的公司**（堵住刷币漏洞）、股数校验、
    总持仓不超过总股本、卖出后市值保底 10000、余额原子扣款
  - 确保 `holdings` 表唯一约束（ON CONFLICT 依赖）
  - `support_company`/`bankrupt_company` RPC 化（支持自己公司也被禁止）
  - `get_my_holdings` 持仓查询
- **股价算法**：股价 = 市值 ÷ 流通股（1000 股），与你后端 buy/sell_stock 完全一致
